$(document).ready(function(){		
	$("#time_in").click(function(){
			...
			var Inimage = $('[name = image_verify]').val();
			console.log("room check" + inRoom);
			$.ajax({
				type: 'POST',
				url: 'http://lormafacultylocator.000webhostapp.com/faculty/time_in.php',
				cache: false,
				data: {device_serial: inDevice_serial, room: inRoom, image: Inimage},
	...
	$("#remarksend").click(function(){
		...
		var remarksroom  = $('[name = room_remarks]').val();
		console.log("REMARKS INPUT: " + remarksInput);
		console.log("Device Serial: " + rdeviceserial);
		console.log("Remarks Room: " + remarksroom);
		$.ajax({
			type: 'POST',
			url: 'http://lormafacultylocator.000webhostapp.com/faculty/time_out_remarks.php',
			cache: false,
			data: {device_serial:rdeviceserial, room:remarksroom, ed_remarks:remarksInput},
			success: function(data){
				...
				while(iterH < hLength){
					if(iterH - 1 == 0){
						...
						if(data.posts[hdataIter].date == data.posts[0].date){
							$("#latest").append('<ons-card><ons-row><ons-col style = "text-align: center" width = "120px">' + data.posts[hdataIter].room + '</ons-col><ons-col style = "align-text: center"><ons-icon icon = "ion-arrow-down-a">' + in12hours + '</ons-icon></ons-col><ons-col style = "align-text: center"><ons-icon icon = "ion-arrow-up-a">' + out12hours + '</ons-icon></ons-col></ons-row></ons-card>');
						}
						else{
							break;
						}
