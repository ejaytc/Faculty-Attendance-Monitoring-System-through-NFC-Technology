				...
				if(iterH - 1 == 0){
					...
					while(hdataIter < hLength){
						if(data.posts[hdataIter].date == data.posts[0].date){
							$("#latest").append('<ons-card><ons-row><ons-col style = "text-align: left" width = "120px">' + data.posts[hdataIter].room + '</ons-col><ons-col style = "align-text: center"><ons-icon icon = "ion-arrow-down-a">' + data.posts[hdataIter].time_in + '</ons-icon></ons-col><ons-col style = "align-text: center"><ons-icon icon = "ion-arrow-up-a">' + data.posts[hdataIter].time_out + '</ons-icon></ons-col></ons-row></ons-card>');
						}
						else{
							break;
						}
	...
	$.ajax({
		...
		success: function(data){			
			...
			/*if(app.roomData == app.roomchecker && app.outremarks != 'null' && app.outremarks != null){
				alert("You already have an entry to this schedule.");
			}
			else{
				...
				while(iterator< iteratorlength){
					...
					if(servertime == sethour){
						...
						if(data.Schedule[iterator].Sched_Room == app.roomData){
							console.log("console: timeOut.display.running: " + data.Schedule[iteratorr].time_out);
							$("#msg").append(designMaker);
...
function displayinputData(roomData, beginTime, endTime){
	...
	$("#schedtimeIO").append("Begin:\t" + beginTime);
	$("#schedtimeIO_").append("End: \t" + endTime);
...
function roompost(){
	...
	displayRoom.setAttribute("style", "text-align: center");
	displayRoom.setAttribute("type", "hidden");
	
	$("#msg").append(devSerialInput);
	$("#msg").append(roomNameInput);
}
